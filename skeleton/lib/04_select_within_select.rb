...
def sparse_continents
  ...
  execute(<<-SQL)
    SELECT
      small_countries.name, small_countries.continent, small_countries.population
    FROM
      (
        SELECT
          *
      ...
      ON
        small_countries.continent = big_countries.continent 
      WHERE
        big_countries.continent IS NULL;
  SQL
end
